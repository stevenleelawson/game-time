...
class Game {
  ...
  update(ctx) {
    ...
    //take these out into a build method push to array, in UPDATE loop over that array and if it's in the array, draw it
  //make drawcities a method
  //logic
      city1.draw(ctx);
      city2.draw(ctx);
      city3.draw(ctx);
    ...
    for (var i = 0; i < 9; i ++) {
      ...
      let targetx = targetArray[targetIndex].x + targetArray[targetIndex].width / 2;
      let targety = targetArray[targetIndex].y;
      // console.log(targetx);
      var x = Math.random() * innerWidth;
      var y = 0;
      var missiles = new Missiles(x, y, targetx, targety);
